...
namespace Hazel {
	...
	void ImGuiLayer::OnEvent(Event& event)
	{
		...
		{
			MouseButtonPressedEvent& e = dynamic_cast<MouseButtonPressedEvent&>(event);
			int button = e.GetMouseButton();
			HZ_CORE_TRACE("{0}", event);
			io.MouseDown[button] = true;

		}
		else if (eventType == EventType::MouseButtonReleased)
		{
			MouseButtonReleasedEvent& e = dynamic_cast<MouseButtonReleasedEvent&>(event);
			int button = e.GetMouseButton();
			HZ_CORE_TRACE("{0}", event);
